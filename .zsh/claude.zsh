...
function claude {
    if [[ "${SOPHIE_CLAUDE_SANDBOX:-}" == "true" ]]; then
        ...
        /usr/local/bin/docker compose -p claude-sandbox exec claude-sandbox zsh -c ". ~/.zshrc && claude ${@}"
    fi
}
...
function claude-karaconnect {
    if [[ "${SOPHIE_CLAUDE_SANDBOX:-}" == "true" ]]; then
        ...
        /usr/local/bin/docker compose -p claude-sandbox exec claude-sandbox zsh -c ". ~/.zshrc && claude-karaconnect ${@}"
    fi
}
